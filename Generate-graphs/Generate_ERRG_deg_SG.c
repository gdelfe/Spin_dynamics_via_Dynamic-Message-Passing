...
double J1,J11,J2,J22,rate ; // couplings
    ...
    for(i=0;i<N;i++){               //note: when calling this function, site[i].degree=0 initially
        for(j=i+1;j<N;j++){
            ...
            if(get_rand()<z){  // get_rand() gives a random numb between [0,1]
                ...
                if(get_rand()<rate){ // generate random symmetric couplings J_{ij}=\pm 1
...
void print_graph_on_file(struct variable *site, char **argv){
    ...
    sprintf(filename,"%s_ERRgraph.dat",argv[8]);  // file to plot the graph using grapviz
    fp_graph=fopen(filename,"w");
    sprintf(filename,"%s_MC_graph.dat",argv[8]);  // file with graph to be used in MC and other simulations
