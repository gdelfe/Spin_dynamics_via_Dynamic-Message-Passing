...
void allocate_memory(struct variable **site, double **m_eq, double ***m_loc, double ****C, double ****C01, double ****C02, double **h);
void read_ERRG(struct variable *site, char **argv);
void read_fields(double *h);

void initialize_ERRG(struct variable *site);
void MC_Glauber(struct variable *site, double beta, double *h);
...
void allocate_memory(struct variable **site, double **m_eq, double ***m_loc, double ****C, double ****C01, double ****C02, double **h){
    ...
    *h =(double *)malloc(N*sizeof(double ));  // vector for the local random field h_i for the RFIM
...
void read_fields(double *h){
    ...
    fp_field=fopen("rand_fields.dat","r");
    if(fp_field==NULL){
        fprintf(stderr,"Problem opening file for random fields\n");
        exit(errno);
    }
    
    for(i=0;i<N;i++)
        fscanf(fp_field,"%lf",&h[i]);
...
void MC_Glauber(struct variable *site, double beta, double *h){
    ...
    for(i=0;i<N;++i){ // cicle on all the i's
        ...
        // rate probability //
        pr= 0.5*(1-site[i].spin_t1*tanh(beta*(sum_neigh+h[i])));
