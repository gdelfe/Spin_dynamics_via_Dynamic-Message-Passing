...
// ./a.out N M0 BETA Tfinal Nsample WSAMPLE S NS NNS
...
void initialize_ERRG_samples(struct variable *site,char *argv[], int i);
...
int main(int argc, char *argv[]){
    ...
    for(i=0;i<Nsample;++i){
        initialize_ERRG_samples(site,argv,i); //initialize all the spins
      //  initialize_ERRG(site);
  ...
  //      if(i%WSAMPLE==0)   printf("sample = %d\n",i);
    ...
    printf("\nN = %d, m0 = %lf, beta = %lf, Tfinal = %d, Nsample = %d, WSAMPLE = %s, file = %s\n\n",N,M0,BETA,Tfinal,Nsample,argv[7],argv[11]);
...
void convert_parameter(int argc, char **argv){
    ...
    if(argc>1){
      ...
      //  printf("%d\t%lf\t%lf\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%s\t%s\n",N,M0,BETA,Tfinal,Teq,Nsample,WSAMPLE,S,NS,NNS,argv[11],argv[12]);
...
//Initialize the spins in a random configuration such that the global magnetization is equal to M0.
// the random configuration is read from a file, because one could start k samples from the exact same configuration.

void initialize_ERRG_samples(struct variable *site, char *argv[], int l){
    ...
    sprintf(filename,"rand_configuration_%s.dat",argv[7]);
    fp_rand=fopen(filename,"r");
    if(fp_rand==NULL){
        fprintf(stderr,"PROBLEM OPENING FILE rand_configuration_%s.dat \n",argv[7]);
        exit(errno);
    }

    
    //change to -1 only a fraction of the spins. The sping which will be changed are read from a file
    for(i=0;i<round((1-M_in)*N);i++){
        fscanf(fp_rand,"%d",&rand_site);
        site[rand_site].spin_t1=-1; //the site is picked from the shuffle array in order to not have repetition
    }
 /*
    if(l<1){
    
        FILE *fp_conf;
        fp_conf=fopen("initial_configuration.dat","w");
