...
void allocate_memory(struct variable **site, double ***m_loc, double ****C, double ****C01, double ****C02);
...
void correlations_t2(struct variable *site, double **m_loc, double ***C, double ***C01, double ***C02, int t);

void print_on_file(double **m_loc, double ***C, double ***C01, double ***C02, char **argv);
double get_rand(void);

int N,c,Nsample,WSAMPLE,Tfinal,S,NS,NNS;
double BETA,M0;
...
void allocate_memory(struct variable **site, double ***m_loc, double ****C, double ****C01, double ****C02){
    ...
    *m_loc =(double **)malloc(N*sizeof(double *)); // vector for the local magnetization for each spin
...
void read_ERRG(struct variable *site, char **argv){
    ...
    sprintf(filename,"%s_MC_graph.dat",argv[10]);
...
void print_on_file(double **m_loc, double ***C, double ***C01, double ***C02, char **argv){
    ...
    FILE *fp_ml, *fp_mtot, *fp_C, *fp_C01, *fp_C02;
    char filename[101];
    
        sprintf(filename,"%s_MC_global_magn.dat",argv[11]);
        fp_mtot=fopen(filename,"w");
    
        sprintf(filename,"%s_MC_local_magn.dat",argv[11]);
        fp_ml=fopen(filename,"w");

        sprintf(filename,"%s_MC_corr.dat",argv[11]);
        fp_C=fopen(filename,"w");
    
        sprintf(filename,"%s_MC_corr01.dat",argv[11]);
        fp_C01=fopen(filename,"w");
        
        sprintf(filename,"%s_MC_corr02.dat",argv[11]);
