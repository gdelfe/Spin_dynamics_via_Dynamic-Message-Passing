...
void print_links(char *argv[]){
    ...
    sprintf(filename,"%s_links.dat",argv[1]);  // file from which I read the links. This file is generated with couplings.awk from the original file containing the structure of the graph, usually ..._MC_graph
    fp_MC=fopen(filename,"r");
    ...
    int nlink[count];  // Generate a random sequence of number with elements in [0,tot_link-1]
    for (i = 0; i < count; i++)
        nlink[i]=i;
    
    for (i = 0; i < count; i++) {    // shuffle array
        temp = nlink[i];
        randomIndex = rand() % count; // random number between 0 and N-1
        
        nlink[i]= nlink[randomIndex];  //shuffle the array to have a random ordering
        nlink[randomIndex] = temp;
    }
