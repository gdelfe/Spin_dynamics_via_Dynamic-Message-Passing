...
/* ------------------------------------------------- */
void initialize_F(struct variable *site, struct dynamic_mp *F0);
void initialize_G(struct variable *site, double **G0);
...
int main(int argc, char *argv[]){
    ...
    get_w(site,w);
    initialize_F(site,F0);
    initialize_G(site,G0);
...
void initialize_F(struct variable *site, struct dynamic_mp *F0){
    ...
    int i, k, si, nj, nk, pk, nn;
    int indF1, mask_si, degree;
    double prod;
    int s[N],mess[N][2];
    ...
    FILE *fp_mess;
    fp_mess=fopen("init_mess.dat","w");
    
    for(i=0;i<N;i++){
        fscanf(fp_conf,"%d",&(s[i]));
        mess[i][0] = (1-s[i])*0.5; // 0 if si = 1, 1 if si = -1
        mess[i][1] = (1+s[i])*0.5; // 1 if si = 1, 0 id si = -1
        fprintf(fp_mess,"mess[%d][0] = %d\nmess[%d][1] = %d\n\n",i,mess[i][0],i,mess[i][1]);
        fprintf(fp_mess,"--------------\n");
    }
    ...
    for(i=0;i<N;i++){    // cicle for all the sites
      ...
      //  printf("\n\n\nnode i = %d  degree = %d \n\n",i,degree);
       ...
       //           printf("--------------- \n");
            ...
            for(si=0;si<2;si++){
                mask_si = si*(1<<(degree-1)); // either 00100 where 1 is in pos degree or it is 00000
                
                k=0;
                do{
                    
                    indF1 = mask_si | k ; // (si | sk sk sk) -- shift to the right because F has one less neigh then w (mask_si is used for both F,w)
                   // showbits(indF1);
           ...
           //         printf("\nF[%d].to[%d][%d] = \n",i,nn,indF1);
                    ...
                    for(nk=0; nk<degree; nk++){
                        if(nk != nj){
                            prod *= mess[site[i].neigh[nk]][((k >> pk) & 1)];
               //             printf(" mess%d[%d][%d]*\t",nk,site[i].neigh[nk],(k >> pk) & 1 );
                            pk++;
                        }
                    }
          //          printf(" mess_i[%d][%d]*\n",i,((indF1 >> (degree-1)) & 1) );
                    prod *= mess[i][(indF1 >>(degree-1)) & 1];
                        
                    //  -------------------------- END RIGHT HAND SIDE
                    
                    F0[i].to[nn][indF1] = prod;  // Assign value of F in the LHS
         //           printf("\n");
                    k++;
...
void initialize_G(struct variable *site, double **G0){
    ...
    int i, j, si, nj, nk, pk, nn;
    int indG1, mask_si, degree;
    double prod;
    int s[N],mess[N][2];
 ...
 //       printf("\n\n\nnode i = %d  degree = %d \n\n",i,degree);
            ...
            for(si=0;si<2;si++){
                ...
                do{
                    ...
                    indG1 = mask_si | j ; // (si | sk sk sk) -- shift to the right because F has one less neigh then w (mask_si is used for both F,w)
                   // showbits(indG1);
           ...
           //         printf("\nG[%d][%d] = \n",i,indG1);
                    ...
                    prod=1;
                    for(nj=0; nj<degree; nj++){
                            prod *= mess[site[i].neigh[nj]][((j >> nj) & 1)];
                    //        printf(" mess%d[%d][%d]*\t",nj,site[i].neigh[nj],(j >> nj) & 1 );
                    }
            //        printf(" mess_i[%d][%d]*\n",i,((indG1 >> degree) & 1) );
                    prod *= mess[i][(indG1 >>degree) & 1];
                    
                    //  -------------------------- END RIGHT HAND SIDE
                    
                    G0[i][indG1] = prod;  // Assign value of F in the LHS
            //        printf(" % lf \n",G0[i][indG1]);
                    j++;
...
void init_magn_corr(FILE *fp_corr,FILE *fp_mtot,FILE *fp_mi, double **m_loc){
    ...
    for(i=0;i<N;i++){
        fscanf(fp_conf,"%d",&(s[i]));
        mess[i][0] = (1-s[i])*0.5; // 0 if si = 1, 1 if si = -1
        mess[i][1] = (1+s[i])*0.5; // 1 if si = 1, 0 if si = -1
    }
    ...
    for(si=0;si<2;si++){
        for(sj=0;sj<2;sj++){
            corr += (2*si-1)*(2*sj-1)*mess[S][si]*mess[NS][sj];
        }
    }
    fprintf(fp_corr,"0\t%lf\t%lf\n",corr,corr-(mess[S][1]-mess[S][0])*(mess[NS][1]-mess[NS][0])); // corr(t,t-1) - m[S](t)m[NS](t-1)
    
    fprintf(fp_mi,"0\t");
    for(i=0;i<N;i++){
        m_tot += mess[i][1]-mess[i][0];
        fprintf(fp_mi,"%lf\t",(double)(mess[i][1]-mess[i][0])); // print local magnetization
        
    }
    fprintf(fp_mi,"\n");
    fprintf(fp_mtot,"0\t%lf\n",m_tot/N); // print global magnetization
    
    m_loc[0][0] = mess[S][1]-mess[S][0];  // local magnetization of spin S at t=0, for later use
    m_loc[1][0] = mess[NS][1]-mess[NS][0]; // local magnetization of spin NS at t=0, for later use
       ...
       //     printf("neigh_j numb[%d] = %d \n",nj,site[i].neigh[nj]);
            ...
            for(si=0;si<2;si++){             
               ...
               //         printf("T[%d].to[%d][%d] = ",i,site[i].neigh[nj],l);
                 ...
                 //           printf("F[%d]w[%d] = %lf + ",indF,indw,F[i].to[nn][indF] * w[i][indw]);
        
                            k++;
                    
                        }while(k<(int)pow(2,degree-1));
                     //  printf("T[%d].to[%d][%d] = %lf\n ",i,site[i].neigh[nj],l,T[i].to[nn][l]);
                        l++;
                    //      printf("\n");
                        /* -------------------------------------------------------------*/
                    }
    ...
    for(i=0;i<N;i++){
        ...
        for(j=0;j<site[i].degree;j++){
            ...
            for(l=0;l<4;l++){
                ...
                if(norm!=0){
                    T[i].to[nn][l]=(double)T[i].to[nn][l]/norm;    // T(1|s_i',s_j')/norm
                    T[i].to[nn][l+4]=(double)T[i].to[nn][l+4]/norm;  // T(-1|s_i',s_j')/norm
                }
            }
...
void allocate_DMP(struct variable *site, struct dynamic_mp **F, struct dynamic_mp **F0, struct dynamic_mp **T, double ***G, double ***G0, double ***w, double ***m_loc){
    ...
    *m_loc=(double **)malloc(2*sizeof(double *)); // local magn for spin S and NS
    ...
    (*m_loc)[1]=(double*)malloc(2*sizeof(double)); // local magn for spin NS at time t and t-1
