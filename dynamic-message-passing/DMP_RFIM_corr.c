...
void allocate_DMP(struct variable *site, struct dynamic_mp **F, struct dynamic_mp **F0, struct dynamic_mp **T, double ***G, double ***G0, double ***w, double ***m_loc, double **h);
...
/* ------------------------------------------------- */
void get_w(struct variable *site, double **w, double *h);
/* ------------------------------------------------- */
...
int main(int argc, char *argv[]){
    ...
    allocate_DMP(site,&F,&F0,&T,&G,&G0,&w,&m_loc,&h); //Allocate memory for Dynamic Message Passing algorithm
    read_fields(h);
...
// The function w depends on the following variables w(si| {sj})
    ...
    for(i=0;i<N;i++){
        ...
        for(si=0;si<2;si++){
            ...
            for(cj=0;cj<(int)pow(2,degree);cj++){ // for all the possible configuration
                ...
                w[i][indw] = exp(BETA*(2*si-1)*(sum+h[i]))/(2*cosh(BETA*(sum+h[i])));
                // printf("h[%d] = %lf\n",i,h[i]);
            }
        }
    }
